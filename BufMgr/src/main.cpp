...
void test4()
{
	...
	try
	{
		...
		//throw new PageNotPinnedException(file4ptr->filename(), i, (FrameId)0);
...
void test7()
{
	...
	for (i = 1; i <= num; i++)
		bufMgr->unPinPage(file1ptr, i, true);
	bufMgr->setBuffer(file1ptr, 1, false);

	try
	{
		bufMgr->flushFile(file1ptr);
		PRINT_ERROR("ERROR :: Not valid buffer. Exception should have been thrown before execution reaches this point.");
	}
	catch(BadBufferException &e)
	{
	}
	std::cout << "Test 7 passed" << "\n";
	bufMgr->setBuffer(file1ptr, 1, true);
	bufMgr->flushFile(file1ptr);
}
