...
bool NestedLoopJoinOperator::execute(int numAvailableBufPages,
  ...
  // TODO: Execute the join algorithm
  Page* Lbuffers[numAvailableBufPages - 1];
  Page* Rbuffer;
  vector<Attribute> commonAttributes = getCommonAttributes(leftTableSchema, rightTableSchema);
  for (FileIterator FLIter = leftTableFile.begin(); FLIter != leftTableFile.end();)
  {
    int i = 0;
    for (i = 0; i < numAvailableBufPages - 1 && FLIter != leftTableFile.end(); ++i,++FLIter)
    {
      Page Lpage = *FLIter;
      bufMgr->readPage(&leftTableFile, Lpage.page_number(), Lbuffers[i]);
      if (numUsedBufPages < numAvailableBufPages)
        numUsedBufPages++;
      numIOs++;
    }
    for (FileIterator FRIter = rightTableFile.begin(); FRIter != rightTableFile.end(); ++FRIter)
    {
      Page Rpage = *FRIter;
      bufMgr->readPage(&rightTableFile, Rpage.page_number(), Rbuffer);
      if (numUsedBufPages < numAvailableBufPages)
        numUsedBufPages++;
      numIOs++;
      for(PageIterator PRIter = Rbuffer->begin(); PRIter != Rbuffer->end(); ++PRIter)
      {
        string Rrecord = *PRIter;
        string Rcommon = construct_search_key(Rrecord, commonAttributes, rightTableSchema);
        for (int j = 0; j < i; ++j)
          for (PageIterator PLIter = Lbuffers[j]->begin(); PLIter != Lbuffers[j]->end(); ++PLIter)
          {
            string Lrecord = *PLIter;
            string Lcommon = construct_search_key(Lrecord, commonAttributes, leftTableSchema);
            if (Lcommon == Rcommon)
            {
              string rst = joinTuples(Lrecord, Rrecord, leftTableSchema, rightTableSchema);
              HeapFileManager::insertTuple(rst, resultFile, bufMgr);
              numResultTuples++;
            }
          }
      }
      bufMgr->unPinPage(&rightTableFile, Rpage.page_number(), false);
    }
  }
